...
test("Explorar shows two distinct sections: Universo Tony and Otros activos", async ({
  page,
}) => {
  ...
  // A curated asset (Apple) appears under "Universo Tony" — find its card and confirm
  // the Tony badge renders.
  const universeSection = page
    .getByRole("heading", { name: "Universo Tony" })
    .locator("xpath=following-sibling::*[1]/following-sibling::div[1]");
  await expect(universeSection.getByText("Apple Inc.")).toBeVisible();

  // A non-curated asset (Bitcoin) appears under "Otros activos", not under "Universo Tony".
...
test("filtering to a category with no curated matches hides the Universo Tony heading", async ({
  page,
}) => {
  await seedProfile(page);
  await page.goto("/explorar");
  ...
  await expect(page.getByRole("heading", { name: "Universo Tony" })).not.toBeVisible();
  await expect(page.getByRole("heading", { name: "Otros activos" })).toBeVisible();
  await expect(page.getByText("Bitcoin", { exact: true })).toBeVisible();
});

test("a curated asset's detail page shows Seguimiento de Tony with thesis and review history", async ({
  page,
}) => {
  await seedProfile(page);
  await page.goto("/explorar");
...
test("a non-curated asset's detail page has no Seguimiento de Tony section", async ({ page }) => {
  await seedProfile(page);
  await page.goto("/explorar");
  await page.getByText("Bitcoin", { exact: true }).click();

  await expect(page.getByText("Seguimiento de Tony")).not.toBeVisible();
});
